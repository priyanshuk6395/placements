      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'previewing' && (
            <motion.div key="previewing" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center">
              ...
              <p className="text-indigo-300 animate-pulse font-medium">Analyzing Upload Impact...</p>
          ...
          {status === 'confirm' && previewSummary && (
            <motion.div key="confirm" initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full space-y-4">
              <div className="text-center">
                <p className="text-indigo-200 font-bold text-base">Confirm Bulk Upload</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3">
                  <p className="text-[10px] uppercase tracking-wider text-emerald-300/80">Will Add</p>
                  <p className="text-xl font-black text-emerald-300">{previewSummary.willInsert}</p>
                </div>
                <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
                  <p className="text-[10px] uppercase tracking-wider text-amber-300/80">Will Update</p>
                  <p className="text-xl font-black text-amber-300">{previewSummary.willUpdate}</p>
                </div>
              </div>
              ...
              {warnings.length > 0 && (
                <div className="w-full max-h-28 overflow-y-auto custom-scrollbar bg-black/20 rounded-lg p-3 border border-amber-500/20">
                  <p className="text-[10px] uppercase tracking-wider text-amber-300 mb-2">Warnings</p>
        ...
        {status === 'error' && (
          <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center w-full">
            ...
            <p className="font-bold text-rose-300 text-lg">Upload Failed</p>
            <p className="text-rose-200/80 text-sm text-center mb-4">{errorMessage || 'Something went wrong while uploading.'}</p>
            <button
              type="button"
              onClick={() => {
                setStatus('idle');
              ...
              className="flex items-center gap-2 px-4 py-2 bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 rounded-xl text-sm font-semibold transition-colors"
            >
