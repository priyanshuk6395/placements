      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-950 border border-white/10 text-sm text-slate-200 placeholder:text-slate-500 outline-none focus:border-indigo-500/40"
          />
        </div>
      </div>
            ...
            <table className="w-full text-left text-[11px] min-w-[760px]">
              ...
              <tbody className="divide-y divide-white/5">
                {pagedRows.map((log) => {
                  ...
                  return (
                    <Fragment key={rowKey}>
                      <tr key={rowKey} className="text-slate-300 hover:bg-white/[0.02]">
                        ...
                        <td className="py-3 pr-4 font-bold text-white">{visitor?.name || "-"}</td>
                        <td className="py-3 pr-4 text-slate-300">{fmtDate(log.timestamp)}</td>
                        <td className="py-3 pr-4">{Math.round(log.duration || 0)}s</td>
                        <td className="py-3 pr-4 max-w-[260px] truncate" title={String(log.path || "-")}>{String(log.path || "-")}</td>
                        <td className="py-3 pr-0 text-right">
                          <button
                            type="button"
                            onClick={() => setExpandedRows((prev) => ({ ...prev, [rowKey]: !prev[rowKey] }))}
                            className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-[10px] font-black uppercase tracking-wide"
                          >
                            {expanded ? "Hide" : "Show"}
                          </button>
                        </td>
                          ...
                          <td colSpan={7} className="px-3 py-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                              <div className="p-2.5 rounded-lg bg-black/20 border border-white/5">
                                <p className="text-slate-500 uppercase tracking-wide text-[10px] font-bold">Device</p>
                                <p className="mt-1 break-all">{String(log.userAgent || "Unknown")}</p>
                              </div>
                              <div className="p-2.5 rounded-lg bg-black/20 border border-white/5">
                                <p className="text-slate-500 uppercase tracking-wide text-[10px] font-bold">Location</p>
                                <p className="mt-1">{String(log.city || "Unknown")}, {String(log.region || "Unknown")}, {String(log.country || "Unknown")}</p>
                              </div>
                              <div className="p-2.5 rounded-lg bg-black/20 border border-white/5">
                                <p className="text-slate-500 uppercase tracking-wide text-[10px] font-bold">Duration</p>
                                <p className="mt-1">{Math.round(log.duration || 0)} seconds</p>
                              </div>
                              <div className="p-2.5 rounded-lg bg-black/20 border border-white/5">
                                <p className="text-slate-500 uppercase tracking-wide text-[10px] font-bold">Path</p>
          ...
          <div className="mt-5 flex items-center justify-between gap-3 text-xs text-slate-400">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1.5 rounded-lg bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200"
              >
                Prev
              </button>
              <span className="text-slate-300 font-bold">Page {page} / {totalPages}</span>
              <button
                type="button"
